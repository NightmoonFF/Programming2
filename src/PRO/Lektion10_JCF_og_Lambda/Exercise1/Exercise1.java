...
*/
    ...
    public static void print() {
        ...
        // a) Find the first person in the persons list with an age of 44
        System.out.println("a) " + findFirst(persons, person -> person.getAge() == 44));

        // b) Find the first person in the persons list with a name starting with 'S'
        System.out.println("b) " + findFirst(persons, person -> person.getName().charAt(0) == 'S'));

        // c) Find the first person in the persons list with a name containing more than one "i"
        System.out.println(
                "c) " + findFirst(persons, person -> person.getName().chars().filter(ch -> ch == 'i').count() >= 2));

        // d) Find the first person in the persons list with an age equal to the length of the name.
        System.out.println("d) " + findFirst(persons, person -> person.getName().length() == person.getAge()));

        // e) Program a findAll() method. Its signature and implementation must fit the use of --this--
        ...
        // g) Find all persons with name starting with 'S'.
        System.out.println("g) " + findAll(persons, p -> p.getName().substring(0, 1).contains("S")));

        // h) Find all persons with name of length 5.
        System.out.println("h) " + findAll(persons, p -> p.getName().length() == 5));

        // i) Find all persons with name of length at least 6 and age below 40
        System.out.println("h) " + findAll(persons, p -> p.getName().length() > 5 && p.getAge() < 40));
